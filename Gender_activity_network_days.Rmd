...
#manually changing the quantiles for network
...
Days
```{r}
danish_p_gender$duration <- as.numeric(cut2(danish_p_gender$days, g=10))

days_gender <- danish_p_gender %>% #making a df for privacy size with dominance type
  group_by(duration, dominance) %>% 
  summarise(n = n(), percentage = round((n/length(danish_p_gender$duration)*100), digits = 3))

days_gender$dominance <- as.factor(days_gender$dominance)
days_gender$duration <- as.factor(days_gender$duration)
...
#plots
g_days <- ggplot(data = days_gender, aes(x = duration, y = percentage, fill = factor(dominance)))

g_days + geom_bar(stat = "identity",width=0.9) + scale_fill_brewer(palette="Paired") + xlab("Duration lenght") + ylab("%") + labs(fill = "Group") + labs(title = "Network size and gender") + labs(caption = "Group: 0 = equally dominant, 1 = male dominant, 2 = female dominant ") + geom_text(aes(label = n), position = position_stack(vjust = 0.8))
...
privacy_cor <- select(male_female,
                      ...
                      activity,
                      dominance)
head(ten_versus_ninety)
privacy_cor <- select(ten_versus_ninety,
                      privacy,
                      ten_perc_activity_post,
                      ten_perc_activity_comment,
                      ten_perc_network,
                      dominance)
...
Last 10% against 90%
```{r}
ten_versus_ninety <- danish_p_gender
ten_versus_ninety$ten_perc_activity_post <- ifelse(ten_versus_ninety$activity_post == 10, 1, 0)
ten_versus_ninety$ten_perc_activity_comment <- ifelse(ten_versus_ninety$activity_comment == 10, 1, 0)
ten_versus_ninety$ten_perc_network <- ifelse(ten_versus_ninety$network == 10, 1, 0)

activity_post_gender_ten <- ten_versus_ninety %>% #making a df for privacy size with dominance type
  group_by(ten_perc_activity_post, dominance) %>% 
  summarise(n = n(), percentage = round((n/length(ten_versus_ninety$ten_perc_activity_post)*100), digits = 3))

# activity_post_gender_ten$ten_perc_activity_post <- as.factor(activity_post_gender_ten$dominance)
# activity_post_gender_ten$ten_perc_activity_post <- as.factor(activity_post_gender_ten$ten_perc_activity_post)

#plots
g_activity_ten <- ggplot(data = activity_post_gender_ten, aes(x = ten_perc_activity_post, y = percentage, fill = factor(dominance)))

g_activity_ten + geom_bar(stat = "identity",width=0.9) + scale_fill_brewer(palette="Paired") + xlab("Actvity size") + ylab("%") + labs(fill = "Group") + labs(title = "Activity size and gender - comment") + labs(caption = "Group: 0 = equally dominant, 1 = male dominant, 2 = female dominant ") + geom_text(aes(label = n), position = position_stack(vjust = 0.8))
```
