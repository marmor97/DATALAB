...
```{r}
#quantiles of 10% for network size
quantiles_10 <- quantile(danish_p_gender$total_unique_P_C, prob = c(0.10, 0.20, 0.30, 0.40, 0.50, 0.60, 0.70, 0.80, 0.90, 1))
quantiles_10

#quantiles of 10% for activity
activity_quantiles_10 <- quantile(danish_p_gender$post_comment_total, prob = c(0.10, 0.20, 0.30, 0.40, 0.50, 0.60, 0.70, 0.80, 0.90, 1))
activity_quantiles_10
```
...
#Creating an empty column
danish_p_gender$network <- c(0)

for (i in (1:nrow(danish_p_gender))) {
  if (danish_p_gender$total_unique_P_C[i] > 0){
    danish_p_gender$network[i] <- 0  
    }
  if (danish_p_gender$total_unique_P_C[i] > quantiles_10[1]){
    danish_p_gender$network[i] <- 1 
  }
  if (danish_p_gender$total_unique_P_C[i] > quantiles_10[2]){
    danish_p_gender$network[i] <- 2 
  }
  if (danish_p_gender$total_unique_P_C[i] > quantiles_10[3]){
    danish_p_gender$network[i] <- 3 
    }
  if (danish_p_gender$total_unique_P_C[i] > quantiles_10[4]){
    danish_p_gender$network[i] <- 4 
  }
  if (danish_p_gender$total_unique_P_C[i] > quantiles_10[5]){
    danish_p_gender$network[i] <- 5 
  }
  if (danish_p_gender$total_unique_P_C[i] > quantiles_10[6]){
    danish_p_gender$network[i] <- 6 
  }
  if (danish_p_gender$total_unique_P_C[i] > quantiles_10[7]){
    danish_p_gender$network[i] <- 7 
  }
  if (danish_p_gender$total_unique_P_C[i] > quantiles_10[8]){
    danish_p_gender$network[i] <- 8 
    }
  if (danish_p_gender$total_unique_P_C[i] > quantiles_10[9]){
    danish_p_gender$network[i] <- 9 
  }
}
...
Activity 10% groups
```{r}
danish_p_gender$activity <- c(0)


for (i in (1:nrow(danish_p_gender))) {
  if (danish_p_gender$post_comment_total[i] > 0){
    danish_p_gender$activity[i] <- 0  
    }
  if (danish_p_gender$post_comment_total[i] > activity_quantiles_10[1]){
    danish_p_gender$activity[i] <- 1 
  }
  if (danish_p_gender$post_comment_total[i] > activity_quantiles_10[2]){
    danish_p_gender$activity[i] <- 2 
  }
  if (danish_p_gender$post_comment_total[i] > activity_quantiles_10[3]){
    danish_p_gender$activity[i] <- 3 
    }
  if (danish_p_gender$post_comment_total[i] > activity_quantiles_10[4]){
    danish_p_gender$activity[i] <- 4 
  }
  if (danish_p_gender$post_comment_total[i] > activity_quantiles_10[5]){
    danish_p_gender$activity[i] <- 5 
  }
  if (danish_p_gender$post_comment_total[i] > activity_quantiles_10[6]){
    danish_p_gender$activity[i] <- 6 
  }
  if (danish_p_gender$post_comment_total[i] > activity_quantiles_10[7]){
    danish_p_gender$activity[i] <- 7 
  }
  if (danish_p_gender$post_comment_total[i] > activity_quantiles_10[8]){
    danish_p_gender$activity[i] <- 8 
    }
  if (danish_p_gender$post_comment_total[i] > activity_quantiles_10[9]){
    danish_p_gender$activity[i] <- 9 
  }
}
...
network_privacy$provacy <- as.factor(network_gender$dominance)
...
#there might be gender differences in preffered social network size and level of activity
hist(privacy_cor$dominance)
```
