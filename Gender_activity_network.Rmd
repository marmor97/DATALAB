...
```{r}
nrow(filter(danish_p_gender, network == 10 & activity == 10))/nrow(filter(danish_p_gender, activity == 10))*100
nrow(filter(danish_p_gender, network == 10 & activity == 10))/nrow(filter(danish_p_gender, network == 10))*100

#67.29242% overlap with activity 10%
#66.19318% overlap with network 10%
```

Removing comment
```{r}
#repeating steps with comment 
danish_p_gender$activity_comment <- as.numeric(cut2(danish_p_gender$comment_total, g=10))

actvity_comment_gender <- danish_p_gender %>% #making a df for privacy size with dominance type
  group_by(dominance, activity_comment) %>% 
  summarise(n = n(), percentage = round((n/length(danish_p_gender$activity_comment)*100), digits = 3))

actvity_comment_gender$dominance <- as.factor(actvity_comment_gender$dominance)
actvity_comment_gender$activity_comment <- as.factor(actvity_comment_gender$activity_comment)

#plots
g_activity <- ggplot(data = actvity_comment_gender, aes(x = activity_comment, y = percentage, fill = factor(dominance)))
